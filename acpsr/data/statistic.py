...
def start_statistic():
    ...
    for _split in splits:
        ...
        with open(os.path.join(dir_path,_split+".json"), 'r') as fp:
            ...
            individual_count += [sd["wav"].split('/')[4] for sd in data_json]
            acupoint_count += [sd["wav"].split('/')[5][:-4] for sd in data_json]
    ...
    print(f'individual count: {len(set(individual_count))}')
    print(f'acupoint count: {len(set(acupoint_count))}')
