...
if __name__ == "__main__":
    ...
    for pres, opt in tqdm.tqdm(raw_pres):
        all_gen += pres_generate((pres, []), all_words, max_permut=4, repeat=3)
        all_gen += pres_generate((pres, opt), all_words, max_permut=4, repeat=3)
        #for item in opt:
            #all_gen += pres_generate((pres, item), all_words, max_permut=2, repeat=2)
